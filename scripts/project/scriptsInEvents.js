...
const scriptsInEvents = {
			...
			for (let index1 = 0; index1 < all_cars.length - 1; index1++) {
				for (let index2 = 0; index2 < all_cars.length; index2++) {
					...
					if (col_data.collided) {
						...
						for (let i = 0; i < 2; i++) {
							cars[i].setAnimation("Colliding");
							cars[i].x -= col_data.overlap.x / 2 * (-i * 2 + 1);
							cars[i].y -= col_data.overlap.y / 2 * (-i * 2 + 1);
							cars[i].behaviors.Car.setImpulse(new_v[i][0], new_v[i][1]);
							cars[i].behaviors.Car.setVector(new_v[i][0], new_v[i][1]);
							//cars[i].behaviors.Car.spin = new_av[i];
						}
					} else for (let car of cars) car.setAnimation("NoColliding");
		...
		async Gamesheet_Event92_Act2(runtime, localVars)
		{
		...
		async Gamesheet_Event100_Act2(runtime, localVars)
		{
